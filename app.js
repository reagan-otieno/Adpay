...
async function bootApp() {
  ...
  auth.onAuthStateChanged(async user => {
    if (user) {
      ...
      } else { hideLoader(); showPage('landing'); }
    } else { hideLoader(); showPage('landing'); }
  });
...
async function logout() {
  ...
  if (isFirebaseReady) await fbLogout().catch(()=>{});
...
async function onUserLoggedIn() {
  ...
  if (isFirebaseReady) {
    unsubUser = watchUser(currentUser.uid, profile => {
      currentUser = profile;
      watchedAdsSet = new Set(profile.watchedAds||[]);
      updateDashStats(); renderTodayAds(); renderActivity();
    });
    unsubAds = watchAds(ads => { cache.ads=ads; renderAdsGrid(); renderTodayAds(); });
    unsubTxns = watchUserTransactions(currentUser.uid, txns => { cache.transactions=txns; renderUserTransactions(); renderActivity(); renderHistory(); });
    setUserPresence(currentUser.uid);
  } else {
    cache.ads = DEMO_ADS;
    cache.transactions = DEMO_TRANSACTIONS.filter(t=>t.uid===currentUser.uid);
    watchedAdsSet = new Set(currentUser.watchedAds||[]);
    renderAdsGrid(); renderTodayAds(); renderActivity(); renderUserTransactions(); renderHistory();
  }
  updateNotifBadge(); hideLoader();
}
...
async function completeAd() {
  ...
  try {
    if (isFirebaseReady) {
      await recordAdView(currentUser.uid, ad.id, ad.name, ad.pay);
    } else {
      currentUser.balance     = parseFloat(((currentUser.balance||0)+ad.pay).toFixed(2));
      currentUser.totalEarned = parseFloat(((currentUser.totalEarned||0)+ad.pay).toFixed(2));
      currentUser.adsWatched  = (currentUser.adsWatched||0)+1;
      currentUser.watchedAds  = [...(currentUser.watchedAds||[]), ad.id];
      DEMO_TRANSACTIONS.unshift({id:'TXN'+Date.now(),uid:currentUser.uid,type:'earning',amount:ad.pay,method:'Ad Watch',status:'completed',desc:ad.name+' Ad',date:new Date().toISOString()});
      cache.transactions = DEMO_TRANSACTIONS.filter(t=>t.uid===currentUser.uid);
      updateDashStats(); renderTodayAds(); renderAdsGrid(); renderActivity(); renderUserTransactions(); renderHistory();
    }
    addNotification('💰','Ad Reward Earned!','+$'+ad.pay.toFixed(2)+' from '+ad.name);
  } catch(e) { console.error('completeAd:',e); }
...
async function processDeposit() {
  ...
  try {
    if (isFirebaseReady) { await updateUser(currentUser.uid,{balance:firebase.firestore.FieldValue.increment(usdAmt)}); await addTransaction(currentUser.uid,'deposit',usdAmt,method,'Deposit via '+method,'completed'); }
    else {
      currentUser.balance=parseFloat(((currentUser.balance||0)+usdAmt).toFixed(2));
      DEMO_TRANSACTIONS.unshift({id:'TXN'+Date.now(),uid:currentUser.uid,type:'deposit',amount:usdAmt,method,status:'completed',desc:'Deposit via '+method,date:new Date().toISOString()});
      cache.transactions=DEMO_TRANSACTIONS.filter(t=>t.uid===currentUser.uid);
      updateDashStats(); renderUserTransactions();
    }
    document.getElementById('deposit-amt').value=''; updateDepositConversion();
    const display=currency==='USD'?'$'+usdAmt.toFixed(2):formatCurrency(amount,currency)+' (≈$'+usdAmt.toFixed(2)+' USD)';
...
async function processWithdraw() {
  ...
  try {
    if (isFirebaseReady) { await updateUser(currentUser.uid,{balance:firebase.firestore.FieldValue.increment(-amount)}); await addTransaction(currentUser.uid,'withdrawal',amount,method,'Withdrawal to '+method,'pending'); }
    else {
      currentUser.balance=parseFloat(((currentUser.balance||0)-amount).toFixed(2));
      DEMO_TRANSACTIONS.unshift({id:'TXN'+Date.now(),uid:currentUser.uid,type:'withdrawal',amount,method,status:'pending',desc:'Withdrawal to '+method,date:new Date().toISOString()});
      cache.transactions=DEMO_TRANSACTIONS.filter(t=>t.uid===currentUser.uid);
      updateDashStats(); renderUserTransactions();
    }
    document.getElementById('withdraw-amt').value=''; updateWithdrawConversion();
...
async function renderLeaderboard() {
  const podiumEl=document.getElementById('lb-podium'), listEl=document.getElementById('lb-full-list');
  if (!podiumEl||!listEl) return;
  podiumEl.innerHTML='<div class="skeleton" style="height:120px;border-radius:12px"></div>';
  listEl.innerHTML=[1,2,3,4,5].map(()=>'<div class="skeleton sk-line wide" style="height:48px;margin-bottom:8px"></div>').join('');
  let users=[];
  try {
    if (isFirebaseReady) { const snap=await db.collection('users').orderBy('totalEarned','desc').limit(20).get(); users=snap.docs.map(d=>({uid:d.id,...d.data()})); }
    else users=[...DEMO_USERS].sort((a,b)=>b.totalEarned-a.totalEarned);
  } catch { users=[...DEMO_USERS]; }
  const medals=['🥇','🥈','🥉'], classes=['first','second','third'];
  if (users.length>=3) {
    podiumEl.innerHTML=[users[1],users[0],users[2]].map((u,vi)=>{
      const rank=vi===0?2:vi===1?1:3, isMe=currentUser&&u.uid===currentUser.uid;
      return `<div class="podium-slot ${classes[rank-1]}"><div class="podium-avatar">${initials(u.name)}</div><div class="podium-name">${u.name.split(' ')[0]}${isMe?' (You)':''}</div><div class="podium-earn">$${(u.totalEarned||0).toFixed(2)}</div><div class="podium-block">${medals[rank-1]}</div></div>`;
    }).join('');
  }
  listEl.innerHTML=users.map((u,i)=>{
    const isMe=currentUser&&u.uid===currentUser.uid;
    return `<div class="lb-row ${isMe?'me':''}"><div class="lb-rank">${i<3?medals[i]:'#'+(i+1)}</div><div class="lb-av">${initials(u.name)}</div><div class="lb-info"><div class="lb-name">${u.name}${isMe?' <span style="color:var(--accent);font-size:.72rem">(You)</span>':''}</div><div class="lb-tier">${getTierBadge(u.tier)}</div></div><div style="text-align:right"><div class="lb-earn">$${(u.totalEarned||0).toFixed(2)}</div><div class="lb-ads">${u.adsWatched||0} ads</div></div></div>`;
  }).join('');
}
